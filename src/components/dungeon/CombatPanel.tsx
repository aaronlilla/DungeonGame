                ...
                .map((enemy) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div style={{ 
                        display: 'flex', 
                        flexDirection: isBoss ? 'column' : 'row',
                        alignItems: isBoss ? 'stretch' : 'center', 
                        gap: isBoss ? '1.2rem' : '0.6rem', 
                        marginBottom: isBoss ? '1rem' : '0.4rem', 
                        position: 'relative', 
                        zIndex: 2 
                      }}>
                        {/* Boss Header - Vertical layout with icon on top */}
                        {isBoss && (
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                            {/* Enemy Image/Icon Frame - Grand style for bosses */}
                            {(() => {
                              // Check if this is a final boss (not miniboss)
                              const isFinalBoss = enemy.id === 'final_boss' || enemy.id === 'final_boss_twin';
                              return (
                                <div style={{
                                  width: isFinalBoss ? '120px' : '80px',
                                  height: isFinalBoss ? '120px' : '80px',
                                  borderRadius: '6px',
                                  background: `linear-gradient(145deg, rgba(50, 40, 35, 0.98) 0%, rgba(30, 25, 20, 1) 100%)`,
                                  ...
                                  position: 'relative',
                                  flexShrink: 0,
                                  boxShadow: `inset 0 2px 6px rgba(0,0,0,0.6), 0 0 20px ${colors.primary}50, 0 4px 12px rgba(0,0,0,0.5), 0 8px 16px rgba(0, 0, 0, 0.8)`,
                                  overflow: 'hidden',
                                  backgroundImage: `radial-gradient(circle at center, ${colors.primary}20, transparent 70%)`,
                            ...
                            {/* Boss Name - Full width on its own line */}
                            <div style={{ 
                              fontSize: '1.6rem', 
                              fontWeight: 700,
                              fontFamily: "'Cinzel', Georgia, serif",
                              color: '#f0e6d2',
                              textShadow: `2px 2px 4px rgba(0,0,0,0.8), 0 0 10px ${colors.primary}40`,
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem' }}>
                              <div style={{
                                ...
                                fontWeight: 700,
                                fontFamily: "'Cinzel', Georgia, serif",
                                textShadow: `0 0 8px ${colors.accent}60, 1px 1px 2px rgba(0,0,0,0.8)`,
                              }}>
                                {enemy.type === 'boss' ? '★ BOSS' : '◆ MINIBOSS'}
                              </div>
                              ...
                              <span style={{ 
                                ...
                                {isDying ? '0' : Math.round(enemy.health).toLocaleString()}
                              </span>
                            </div>
                          </div>
                        )}
                        
                        {/* Non-Boss Layout (Original) */}
                        {!isBoss && (
                          <>
                            {/* Enemy Image/Icon Frame */}
                            <div style={{
                              width: '32px',
                              height: '32px',
                              borderRadius: '2px',
                              background: `linear-gradient(145deg, rgba(40, 35, 30, 0.95) 0%, rgba(25, 22, 18, 0.98) 100%)`,
                      ...
                      <div style={{ 
                        height: isBoss ? '16px' : '8px', 
                        background: isBoss ? 'rgba(0, 0, 0, 0.6)' : 'rgba(0, 0, 0, 0.4)', 
                        borderRadius: isBoss ? '4px' : '1px', 
                        overflow: 'hidden', 
                        position: 'relative',
                      ...
                      {enemy.isCasting && !isDying && (() => {
                        ...
                        let castProgress = 0;
                        const currentTick = Math.floor(combatState.timeElapsed * 10); // Convert seconds to ticks
                        if (enemy.castStartTick !== undefined && enemy.castEndTick !== undefined) {
                          const totalCastTicks = enemy.castEndTick - enemy.castStartTick;
                          if (totalCastTicks > 0) {
                            const elapsedTicks = currentTick - enemy.castStartTick;
                        ...
                        return (
                          <div style={{ marginTop: isBoss ? '0.6rem' : '0.3rem' }}>
                            ...
                            <div style={{ 
                              ...
                              <motion.div 
                                key={`enemy-cast-${enemy.castStartTick}`}
