...
export async function runDungeonCombat(params: DungeonCombatParams): Promise<DungeonRunResult | null> {
  ...
  for (let pullIdx = 0; pullIdx < routePulls.length; pullIdx++) {
    ...
    // Travel
    updateCombatState(prev => ({ ...prev, phase: 'traveling', currentPullIndex: pullIdx, combatLog: [...prev.combatLog, { timestamp: totalTime, type: 'travel', source: '', target: '', message: `🚶 Moving to Pull #${pullIdx + 1}... (${travelTime.toFixed(1)}s)` }] }));
    ...
    updateCombatState(prev => ({
      ...
      combatLog: [...prev.combatLog, { timestamp: totalTime, type: hasGateBoss ? 'boss' : 'pull', source: '', target: '', message: hasGateBoss ? `👑 GATE BOSS: ${pullEnemies.length} enemies!` : `⚔️ PULL #${pullIdx + 1}: ${pullEnemies.length} enemies!` }]
    }));
    ...
    while (currentEnemies.some(e => e.health > 0) && teamStates.some(m => !m.isDead) && !timedOut) {
      ...
      if (!teamStates.some(m => !m.isDead)) {
        updateCombatState(prev => ({ ...prev, phase: 'defeat', combatLog: [...prev.combatLog, { timestamp: totalTime, type: 'phase', source: '', target: '', message: '💀 WIPE! Your party has been defeated.' }] }));
        ...
        for (const enemy of aliveEnemies) {
          ...
          if (enemy.isCasting && enemy.castStartTime && enemy.castTotalTime) {
            ...
            if (elapsedSeconds >= enemy.castTotalTime) {
              ...
              if (target && !target.isDead) {
                ...
                const jitterX = (Math.random() * 80) - 40;
                const jitterY = (Math.random() * 60) - 30;
                const icon = isCrushingBlow ? '💥' : isAuto ? '🗡️' : '🔮';
          ...
          if (enemy.autoAttackTimer <= 0) {
            ...
            if (aaTarget) {
              ...
              updateCombatState(prev => ({ ...prev, combatLog: [...prev.combatLog, { timestamp: totalTime, type: 'ability', source: enemy.name, target: '', message: `⚔️ ${enemy.name} prepares an auto attack...` }] }));
              continue;
            }
          }
          ...
          if (enemy.behavior === 'melee') {
            ...
            if (meleeTarget) {
              ...
              if (Math.random() < evasionChance) {
                // Attack evaded, no damage
                updateCombatState(prev => ({ ...prev, combatLog: [...prev.combatLog, { timestamp: totalTime, type: 'ability', source: enemy.name, target: meleeTarget.name, message: `💨 ${meleeTarget.name} evades ${enemy.name}'s attack!` }] }));
              } else {
                // PoE armor formula: Armor / (Armor + 10 * Damage)
                const armorMult = calculateArmorReduction(effectiveArmor, rawDamage);
                let damageAfterArmor = Math.floor(rawDamage * armorMult * painSuppMult);
                ...
                if (dmg > 0) {
                  ...
                  const jitterY = (Math.random() * 60) - 30;
                  const floatNum = createFloatingNumber(dmg, blocked ? 'blocked' : 'enemy', currentCombatState.teamPosition.x + jitterX, currentCombatState.teamPosition.y - 40 + jitterY);
                  updateCombatState(prev => ({ ...prev, floatingNumbers: [...prev.floatingNumbers.slice(-20), floatNum], combatLog: [...prev.combatLog, { timestamp: totalTime, type: 'damage', source: enemy.name, target: meleeTarget.name, value: dmg, message: `⚔️ ${enemy.name} hits ${meleeTarget.name} for ${dmg}${blocked ? ' (BLOCKED!)' : ''}!` }] }));
                }
                if (meleeTarget.health <= 0 && !meleeTarget.isDead) {
                  meleeTarget.isDead = true;
                  updateCombatState(prev => ({ ...prev, combatLog: [...prev.combatLog, { timestamp: totalTime, type: 'death', source: '', target: meleeTarget.name, message: `💀 ${meleeTarget.name} has died!` }] }));
                }
              }
            }
              ...
              if (enemy.aoeCooldown <= 0 && aliveMembers.length > 0) {
                ...
                aliveMembers.forEach(member => {
                  ...
                  if (member.health <= 0 && !member.isDead) {
                    member.isDead = true;
                    updateCombatState(prev => ({ ...prev, combatLog: [...prev.combatLog, { timestamp: totalTime, type: 'death', source: '', target: member.name, message: `💀 ${member.name} has died!` }] }));
                  }
                });
            ...
            if (enemy.crushingBlowCooldown <= 0 && tank && !tank.isDead) {
              ...
              updateCombatState(prev => ({ ...prev, combatLog: [...prev.combatLog, { timestamp: totalTime, type: 'ability', source: enemy.name, target: tank.name, message: `💥 ${enemy.name} begins casting Crushing Blow on ${tank.name}... (UNINTERRUPTABLE!)` }] }));
            ...
            if (targetPool.length > 0) {
              ...
              if (Math.random() < evasionChance) {
                ...
                if (dmg > 0) {
                  ...
                  const jitterY = (Math.random() * 60) - 30;
                  const floatNum = createFloatingNumber(dmg, blocked ? 'blocked' : 'enemy', currentCombatState.teamPosition.x + jitterX, currentCombatState.teamPosition.y - 40 + jitterY);
                  updateCombatState(prev => ({ ...prev, floatingNumbers: [...prev.floatingNumbers.slice(-20), floatNum], combatLog: [...prev.combatLog, { timestamp: totalTime, type: 'damage', source: enemy.name, target: target.name, value: dmg, message: `🏹 ${enemy.name} shoots ${target.name} for ${dmg}${blocked ? ' (BLOCKED!)' : ''}!` }] }));
                }
                if (target.health <= 0 && !target.isDead) {
                  target.isDead = true;
                  updateCombatState(prev => ({ ...prev, combatLog: [...prev.combatLog, { timestamp: totalTime, type: 'death', source: '', target: target.name, message: `💀 ${target.name} has died!` }] }));
      ...
      if (currentHealer) {
        ...
        if (currentHealer.isCasting && currentHealer.castStartTime && currentHealer.castTotalTime) {
          ...
          if (!castTarget || castTarget.isDead) {
            ...
            updateCombatState(prev => ({ ...prev, combatLog: [...prev.combatLog, { timestamp: totalTime, type: 'ability', source: currentHealer.name, target: '', message: `❌ ${currentHealer.name}'s heal was cancelled - target died!` }] }));
        ...
        if (!currentHealer.isCasting && healerGcdReady) {
          ...
          // Priority 2: Massive Heal on critically injured (≤50% health)
          else {
      ...
      team.forEach((char) => {
        ...
        if (char.role === 'dps') {
          ...
          if (gcdReady && currentAlive.length > 0) {
            ...
            dpsLogEntries.push({ timestamp: totalTime, type: 'ability', source: char.name, target: targetName || '', message: `🔮 ${char.name} begins casting ${spellChoice}${targetName ? ` on ${targetName}` : ''}...` });
          }
          return;
        }
        ...
        if (aliveEnemies.length > 0) {
          ...
          updateCombatState(prev => ({ 
            ...
            combatLog: [...prev.combatLog, { 
              ...
              message: `✨ ${char.name} Holy Nova hits ${aliveEnemies.length > 1 ? aliveEnemies.length + ' enemies' : aliveEnemies[0].name} for ${finalDamage}${isCrit ? ' CRIT!' : ''} each!` 
            }] 
          }));
        }
        member.gcdEndTime = Date.now() + 1500; // 1.5s GCD
        ...
        if (enemy.health <= 0 && !enemy.isDead) {
          ...
          if (!experienceAwarded.has(enemy.id)) {
            ...
            if (enemyDef) {
              ...
              updateCombatState(prev => ({ ...prev, combatLog: [...prev.combatLog, { timestamp: totalTime, type: 'death', source: '', target: enemy.name, message: `⭐ ${enemy.name} defeated!` }] }));
            }
          } else {
            updateCombatState(prev => ({ ...prev, combatLog: [...prev.combatLog, { timestamp: totalTime, type: 'death', source: '', target: enemy.name, message: `⭐ ${enemy.name} defeated!` }] }));
          }
    ...
    if (healer && !combatRef.current.stop) {
      ...
      while (membersNeedingHealing.length > 0 && !combatRef.current.stop && !timedOut) {
        if (firstHealMessage) {
          updateCombatState(prev => ({ ...prev, phase: 'traveling', combatLog: [...prev.combatLog, { timestamp: totalTime, type: 'ability', source: healer.name, target: '', message: `🩸 ${healer.name} is healing up the party...` }] }));
        ...
        updateCombatState(prev => ({ 
          ...
          floatingNumbers: [...prev.floatingNumbers.slice(-20), floatNum],
          combatLog: [...prev.combatLog, { timestamp: totalTime, type: 'heal', source: healer.name, target: injured.name, value: actualHeal, message: `💊 ${healer.name} casts Healing Wave on ${injured.name} for ${actualHeal}${isCritHeal ? ' CRIT!' : ''}!` }]
        }));
      ...
      if (healer.mana < healer.maxMana * 0.8 && !combatRef.current.stop && !timedOut) {
        ...
        updateCombatState(prev => ({ ...prev, combatLog: [...prev.combatLog, { timestamp: totalTime, type: 'ability', source: healer.name, target: '', message: `🥤 ${healer.name} is drinking...` }] }));
        ...
        updateCombatState(prev => ({ ...prev, combatLog: [...prev.combatLog, { timestamp: totalTime, type: 'buff', source: healer.name, target: '', message: `✨ ${healer.name} finished drinking. Ready!` }] }));
      ...
      if (timedOut) {
        updateCombatState(prev => ({ ...prev, phase: 'defeat', combatLog: [...prev.combatLog, { timestamp: totalTime, type: 'phase', source: '', target: '', message: '⏰ TIME EXPIRED during recovery! Dungeon failed.' }] }));
        setIsRunning(false);
        return null;
      }
    }
    ...
    if (deadMembers.length > 0) {
      ...
      if (!healer) {
        ...
        updateCombatState(prev => ({ ...prev, phase: 'defeat', combatLog: [...prev.combatLog, { timestamp: totalTime, type: 'phase', source: '', target: '', message: '💀 WIPE! Healer is dead and cannot resurrect the party.' }] }));
        setIsRunning(false);
        return null;
      }
      ...
      updateCombatState(prev => ({ 
        ...prev, 
        phase: 'traveling',
        combatLog: [...prev.combatLog, { timestamp: totalTime, type: 'ability', source: healer.name, target: '', message: `🙏 ${healer.name} begins casting Mass Resurrection... (${rezCastTime}s)` }]
      }));
      ...
      if (timedOut) {
        updateCombatState(prev => ({ ...prev, phase: 'defeat', combatLog: [...prev.combatLog, { timestamp: totalTime, type: 'phase', source: '', target: '', message: '⏰ TIME EXPIRED during resurrection! Dungeon failed.' }] }));
        setIsRunning(false);
        return null;
      }
      ...
      updateCombatState(prev => ({ 
        ...
        combatLog: [...prev.combatLog, { timestamp: totalTime, type: 'heal', source: healer.name, target: rezNames, message: `✨ ${healer.name} resurrects ${rezNames}!` }]
      }));
  ...
  if (boss && teamStates.some(m => !m.isDead) && !timedOut && !combatRef.current.stop) {
    ...
    updateCombatState(prev => ({
      ...
      combatLog: [...prev.combatLog, { timestamp: totalTime, type: 'boss', source: '', target: '', message: `👑 FINAL BOSS: ${boss.enemy.name}!` }]
    }));
    ...
    while (bossHealth > 0 && teamStates.some(m => !m.isDead) && !timedOut) {
      ...
      if (!stunActive) {
        ...
        if (bossPulseTimer <= 0) {
          ...
          for (const member of aliveMembers) {
            ...
            const jitterY = (Math.random() * 60) - 30;
            const floatNum = createFloatingNumber(dmg, blocked ? 'blocked' : 'enemy', currentCombatState.teamPosition.x + jitterX, currentCombatState.teamPosition.y - 40 + jitterY);
            updateCombatState(prev => ({ ...prev, floatingNumbers: [...prev.floatingNumbers.slice(-20), floatNum], combatLog: [...prev.combatLog, { timestamp: totalTime, type: 'damage', source: boss.enemy.name, target: member.name, value: dmg, message: `💀 ${boss.enemy.name}'s Pulse hits ${member.name} for ${dmg}${blocked ? ' (BLOCKED!)' : ''}!` }] }));
            if (member.health <= 0) {
              member.isDead = true;
              updateCombatState(prev => ({ ...prev, combatLog: [...prev.combatLog, { timestamp: totalTime, type: 'death', source: '', target: member.name, message: `💀 ${member.name} has died!` }] }));
            }
        ...
        if (bossVolleyCasting && bossVolleyCastStartTime > 0) {
          ...
          if (elapsedMs >= (BOSS_VOLLEY_CAST_TIME * 1000)) {
            ...
            for (const member of aliveMembers) {
              ...
              const jitterY = (Math.random() * 60) - 30;
              const floatNum = createFloatingNumber(dmg, blocked ? 'blocked' : 'enemy', currentCombatState.teamPosition.x + jitterX, currentCombatState.teamPosition.y - 40 + jitterY);
              updateCombatState(prev => ({ ...prev, floatingNumbers: [...prev.floatingNumbers.slice(-20), floatNum], combatLog: [...prev.combatLog, { timestamp: totalTime, type: 'damage', source: boss.enemy.name, target: member.name, value: dmg, message: `⚡ ${boss.enemy.name}'s Volley hits ${member.name} for ${dmg}${blocked ? ' (BLOCKED!)' : ''}!` }] }));
              if (member.health <= 0) {
                member.isDead = true;
                updateCombatState(prev => ({ ...prev, combatLog: [...prev.combatLog, { timestamp: totalTime, type: 'death', source: '', target: member.name, message: `💀 ${member.name} has died!` }] }));
              }
            ...
            updateCombatState(prev => ({ ...prev, combatLog: [...prev.combatLog, { timestamp: totalTime, type: 'ability', source: boss.enemy.name, target: '', message: `⚡ ${boss.enemy.name} unleashes Volley on the party!` }] }));
          }
        }
        ...
        if (bossTankBusterCooldown <= 0 && tank && !bossVolleyCasting) {
          ...
          const jitterY = (Math.random() * 60) - 30;
          const floatNum = createFloatingNumber(dmg, blocked ? 'blocked' : 'enemy', currentCombatState.teamPosition.x + jitterX, currentCombatState.teamPosition.y - 40 + jitterY);
          updateCombatState(prev => ({ ...prev, floatingNumbers: [...prev.floatingNumbers.slice(-20), floatNum], combatLog: [...prev.combatLog, { timestamp: totalTime, type: 'damage', source: boss.enemy.name, target: tank.name, value: dmg, message: `💥 ${boss.enemy.name}'s Tank Buster hits ${tank.name} for ${dmg}${blocked ? ' (BLOCKED!)' : ''}!` }] }));
          if (tank.health <= 0) {
            tank.isDead = true;
            updateCombatState(prev => ({ ...prev, combatLog: [...prev.combatLog, { timestamp: totalTime, type: 'death', source: '', target: tank.name, message: `💀 ${tank.name} has died!` }] }));
          }
        }
        ...
        if (bossVolleyCooldown <= 0 && !bossVolleyCasting && bossTankBusterCooldown > 0) {
          ...
          updateCombatState(prev => ({ ...prev, combatLog: [...prev.combatLog, { timestamp: totalTime, type: 'ability', source: boss.enemy.name, target: '', message: `⚡ ${boss.enemy.name} begins casting Volley...` }] }));
        }
        ...
        if (bossSlamCooldown <= 0 && tank && !bossVolleyCasting) {
          ...
          const jitterY = (Math.random() * 60) - 30;
          const floatNum = createFloatingNumber(dmg, blocked ? 'blocked' : 'enemy', currentCombatState.teamPosition.x + jitterX, currentCombatState.teamPosition.y - 40 + jitterY);
          updateCombatState(prev => ({ ...prev, floatingNumbers: [...prev.floatingNumbers.slice(-20), floatNum], combatLog: [...prev.combatLog, { timestamp: totalTime, type: 'damage', source: boss.enemy.name, target: tank.name, value: dmg, message: `💢 ${boss.enemy.name}'s Slam hits ${tank.name} for ${dmg}${blocked ? ' (BLOCKED!)' : ''}!` }] }));
          if (tank.health <= 0) {
            tank.isDead = true;
            updateCombatState(prev => ({ ...prev, combatLog: [...prev.combatLog, { timestamp: totalTime, type: 'death', source: '', target: tank.name, message: `💀 ${tank.name} has died!` }] }));
          }
      ...
      team.forEach((char) => {
        ...
        if (char.role === 'dps') {
          ...
          if (member.isCasting && member.castStartTime && member.castTotalTime) {
            ...
            if (castComplete) {
              ...
              const abilityName = ability === 'Blow Up' ? '💥 Blow Up' : ability === 'Shadow Bolt' ? '🌑 Shadow Bolt' : '🔥 Fireball';
              updateCombatState(prev => ({ 
                ...prev, 
                combatLog: [...prev.combatLog, { 
                  timestamp: totalTime, 
          ...
          if (gcdReady && bossHealth > 0) {
            ...
            updateCombatState(prev => ({ 
              ...
              combatLog: [...prev.combatLog, { 
                ...
                source: char.name, 
                target: boss.enemy.name, 
                message: `🔮 ${char.name} begins casting ${spellChoice} on ${boss.enemy.name}...` 
              }] 
            }));
          }
          return;
        ...
        if (char.role === 'tank') {
          ...
          updateCombatState(prev => ({ 
            ...
            combatLog: [...prev.combatLog, { 
              ...
              target: boss.enemy.name, 
              value: finalDamage, 
              message: `🛡️ Shield Slam ${char.name} hits ${boss.enemy.name} for ${finalDamage}${isCrit ? ' CRIT!' : ''}!` 
            }] 
          }));
          return;
        }
        ...
        updateCombatState(prev => ({ 
          ...
          combatLog: [...prev.combatLog, { 
            ...
            target: boss.enemy.name, 
            value: finalDamage, 
            message: `✨ Smite ${char.name} hits ${boss.enemy.name} for ${finalDamage}${isCrit ? ' CRIT!' : ''}!` 
          }] 
        }));
      });
      ...
      if (phase === 1 && healthPercent <= 60) {
        phase = 2;
        updateCombatState(prev => ({ ...prev, combatLog: [...prev.combatLog, { timestamp: totalTime, type: 'phase', source: boss.enemy.name, target: '', message: `⚡ ${boss.enemy.name} enters Phase 2!` }] }));
      } else if (phase === 2 && healthPercent <= 30) {
        phase = 3;
        updateCombatState(prev => ({ ...prev, combatLog: [...prev.combatLog, { timestamp: totalTime, type: 'phase', source: boss.enemy.name, target: '', message: `💥 ${boss.enemy.name} ENRAGES!` }] }));
      }
    ...
    if (bossHealth <= 0 && !experienceAwarded.has('final_boss')) {
      ...
      updateCombatState(prev => ({ 
        ...
        combatLog: [...prev.combatLog, { 
          ...
          source: '', 
          target: boss.enemy.name, 
          message: `🎉 ${boss.enemy.name} DEFEATED!` 
        }] 
      }));
    }
  }
  ...
  updateCombatState(prev => ({ ...prev, phase: 'victory', enemies: [], combatLog: [...prev.combatLog, { timestamp: totalTime, type: 'phase', source: '', target: '', message: `🏆 DUNGEON COMPLETE in ${Math.floor(totalTime)}s!` }] }));
